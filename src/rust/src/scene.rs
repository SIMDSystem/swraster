...
pub fn build_ground_geometry(ground_half: f32, out_vertices: &mut RenderVertexList, out_faces: &mut Vec<Face>) {
    out_vertices.clear();
    out_faces.clear();
    let mut add_vertex = |verts: &mut RenderVertexList, x: f32, z: f32, u: f32, vv: f32| -> i32 {
    ...
    let g0 = add_vertex(out_vertices, -ground_half, -ground_half, 0.0, 0.0);
    let g1 = add_vertex(out_vertices, ground_half, -ground_half, 2.0, 0.0);
    let g2 = add_vertex(out_vertices, ground_half, ground_half, 2.0, 2.0);
    let g3 = add_vertex(out_vertices, -ground_half, ground_half, 0.0, 2.0);
    out_faces.push(Face { v0: g0, v1: g2, v2: g1, r: 0.68, g: 0.68, b: 0.68, a: 1.0 });
    out_faces.push(Face { v0: g0, v1: g3, v2: g2, r: 0.68, g: 0.68, b: 0.68, a: 1.0 });
}
...
pub unsafe fn build_tumbling_walls(
    ...
    let mut create_wall = |shape: *mut jolt::Shape, local_pos: Vec3| {
        let id = jolt::jph_body_create_and_add(
            bi,
            shape,
            local_pos,
            Quat::identity(),
    ...
    create_wall(jolt::jph_box_shape_create(full, wall_thick, full), Vec3::new(0.0, -box_half - wall_thick, 0.0));
    create_wall(jolt::jph_box_shape_create(full, wall_thick, full), Vec3::new(0.0, box_half + wall_thick, 0.0));
    create_wall(jolt::jph_box_shape_create(wall_thick, full, full), Vec3::new(-box_half - wall_thick, 0.0, 0.0));
    create_wall(jolt::jph_box_shape_create(wall_thick, full, full), Vec3::new(box_half + wall_thick, 0.0, 0.0));
    create_wall(jolt::jph_box_shape_create(full, full, wall_thick), Vec3::new(0.0, 0.0, -box_half - wall_thick));
    create_wall(jolt::jph_box_shape_create(full, full, wall_thick), Vec3::new(0.0, 0.0, box_half + wall_thick));
}
...
pub unsafe fn build_torus_compound_shape(
    ...
    let builder = jolt::jph_compound_builder_create();
    if builder.is_null() {
        return std::ptr::null_mut();
    }
    let capsule = jolt::jph_capsule_shape_create(half_height, minor_radius);
    for i in 0..num_segments {
        let angle = i as f32 * 2.0 * M_PI / num_segments as f32;
        let x = major_radius * angle.cos();
        let z = major_radius * angle.sin();
        let rot = Quat::s_rotation(Vec3::new(angle.cos(), 0.0, angle.sin()), M_PI * 0.5);
        jolt::jph_compound_builder_add(builder, Vec3::new(x, 0.0, z), rot, capsule);
    }
    let shape = jolt::jph_compound_builder_build(builder);
    jolt::jph_compound_builder_destroy(builder);
    shape
}
...
pub unsafe fn build_teapot_compound_shape(scale: f32, tess: i32) -> *mut jolt::Shape {
    ...
    let mut add_hull = |pts: &[Vec3]| {
        let h = jolt::jph_convex_hull_shape_create(pts.as_ptr(), pts.len() as i32);
        if !h.is_null() {
            jolt::jph_compound_builder_add(builder, Vec3::zero(), Quat::identity(), h);
        }
    };
...
pub unsafe fn populate_scene_instances(
    ...
    let mut create_main_object =
        |rng: &mut Rng, ty: i32, px: f32, py: f32, pz: f32, shape: *mut jolt::Shape, t: i32, mask_counter: &mut i32, insts: &mut Vec<CubeInstance>| {
            ...
            let rx = rng.unit() * 2.0 * M_PI;
            let ry = rng.unit() * 2.0 * M_PI;
            let rz = rng.unit() * 2.0 * M_PI;
            let initial_rotation = Quat::s_euler_angles(Vec3::new(rx, ry, rz));
            inst.body_id = jolt::jph_body_create_and_add(
                bi,
                shape,
                Vec3::new(px, py, pz),
                initial_rotation,
    ...
    for _ in 0..10 {
        ...
        create_main_object(&mut rng, 0, px, py, pz, jolt::jph_box_shape_create(1.0, 1.0, 1.0), tex_main_cube, &mut mask_counter, instances);
    }
    for _ in 0..10 {
        let px = rng.unit() * 10.0 - 5.0;
        let py = rng.unit() * 10.0 - 5.0;
        let pz = rng.unit() * 10.0 - 5.0;
        create_main_object(&mut rng, 1, px, py, pz, jolt::jph_sphere_shape_create(1.3), tex_main_sphere, &mut mask_counter, instances);
    }
    for _ in 0..10 {
        let px = rng.unit() * 10.0 - 5.0;
        let py = rng.unit() * 10.0 - 5.0;
    ...
    for _ in 0..400 {
        ...
        let shape = jolt::jph_sphere_shape_create(0.3);
        let rx = rng.unit() * 2.0 * M_PI;
        let ry = rng.unit() * 2.0 * M_PI;
        let rz = rng.unit() * 2.0 * M_PI;
        let initial_rotation = Quat::s_euler_angles(Vec3::new(rx, ry, rz));
