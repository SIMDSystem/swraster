...
pub static QUAD_PATH_ENABLED: AtomicBool = AtomicBool::new(true);
...
/// or `None` when the segment misses the rect entirely.
#[inline]
...
pub unsafe fn draw_line_depth(
    ...
    unsafe {
        ...
        let mut z1 = z1_in;
        {
            let Some((t_a, t_b)) = clip_line_to_rect(
...
struct LitCtx<'a> {
    ...
    // Selected mip level; None means untextured (vertex-color) shading.
...
unsafe fn shade_lit_fragment(
    ...
    unsafe {
        ...
        let mut final_g;
        let mut final_b;
        if let Some(level) = ctx.tex_level {
...
pub unsafe fn draw_triangle_barycentric_strip(
    ...
    unsafe {
        ...
        let quad_enabled = QUAD_PATH_ENABLED.load(Ordering::Relaxed);
            ...
            while x <= x_max {
                ...
                'scalar: {
                    ...
                    if shader == TriangleShader::LuminaireCone {
                        ...
                        let distal_fade = 0.5 + 0.5 * (PI * cone_t).cos();
...
pub fn build_luminaire_cone_tl(
    ...
    for i in 0..config::LUMINAIRE_CONE_SEGMENTS {
        ...
        let a0 = (2.0 * PI * i as f32) / seg;
        let a1 = (2.0 * PI * (i + 1) as f32) / seg;
