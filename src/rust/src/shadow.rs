...
#[inline]
pub fn shadow_vertex_from_varying(v: &VertexVaryings, out: &mut ShadowVertex) -> bool {
    if v.sq == 0.0 {
        return false;
    }
    let inv_q = 1.0 / v.sq;
    out.x = v.ss * inv_q * (config::SHADOW_MAP_SIZE - 1) as f32;
    out.y = v.st * inv_q * (config::SHADOW_MAP_SIZE - 1) as f32;
    out.z = v.sr * inv_q;
    true
}
...
pub unsafe fn draw_shadow_line_strip(
    ...
    let mut t_a = 0.0f32;
    let mut t_b = 0.0f32;
    if !draw::clip_line_to_rect(
        v0.x, v0.y, v1.x, v1.y, clip_xmin, clip_ymin, clip_xmax, clip_ymax, &mut t_a, &mut t_b,
    ) {
        return;
    }
    let dx_f = v1.x - v0.x;
