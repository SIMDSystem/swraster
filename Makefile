...
# Layout: src/<lang> code; src/web page sources; build/deps shared Jolt+joltc;
# build/<lang> intermediates+bin+app; build/web/<lang> docroot.
SRC_DIR   = src/cpp
WEB_SRC_DIR = src/web
SCRIPT_DIR = scripts
ASSET_DIR = assets
...
# JPH_* defines must match how libJolt.a was built; adding JPH_ENABLE_ASSERTS
# here without asserts in Jolt leaves JPH::AssertFailed unresolved at link.
...
# Codegen flags repeated at the ThinLTO link, where cross-TU opt runs.
...
# emsdk sysroot libc headers (the Zig wasm compile needs these).
...
# Also compiles the Zig/Odin joltc glue. The -O3 at the link below is where
# Binaryen's wasm-opt runs; without it emcc links at -O0.
...
WEB_LDFLAGS = -O3 -pthread -sUSE_PTHREADS=1 -sPROXY_TO_PTHREAD=1 \
  ...
  -g2
# Mount each asset at its bare basename in the virtual FS (loader asks for "baboon.bmp").
...
# kept for the page shell). -O3 at the link is where Binaryen's wasm-opt runs.
...
# Zig bundles LLVM 21; emsdk ships LLVM 23. The split lanes keep Zig's frontend
# + optimizer but re-run codegen with clang-23: Zig emits optimized IR, clang-23
# lowers it. Closed ~all of the web gap; can't touch Zig's LLVM 21 mid-end passes.
LLVM23_BIN = $(abspath $(EMSCRIPTEN_ROOT)/../bin)
LLVM23_CLANG = $(LLVM23_BIN)/clang
# Must match src/zig/build.zig's wasm feature set so IR and codegen agree.
...
# -no-bounds-check: without it every pixel-loop slice index pays a branch.
...
# clang-23 instead of Apple clang, to isolate backend vintage in the native
# ladder. Single-shot unity TU (not the incremental obj/ pipeline): ld64.lld's
# Mach-O LTO rejects clang-23 ThinLTO bitcode on arm64 ("Unsupported stack
# probing method") and emsdk ships no llvm-link, so unity is the only cross-TU
# inlining path. physics_pipeline.cpp's set_physics_qos collides in unity -> own TU.
...
# --- Rust native build ------------------------------------------------------
# cargo + build.rs (compiles joltc, links libJolt.a). CARGO_TARGET_DIR is set
# explicitly so artifacts land under build/rust even if the shell exports its own.
CARGO ?= cargo
...
# Fetch third_party submodules (fresh clones use --recurse-submodules instead).
.PHONY: deps
deps: $(HIGHWAY_HEADER)
...
# Jolt built out-of-tree (source CMakeLists in the submodule, build under build/deps).
...
# --- C++ native (two-step: per-TU ThinLTO bitcode -> ThinLTO link). Incremental;
# -MMD/-MP track header deps.
...
# Zig wasm step 1: Zig emits optimized IR (-lc for the __main_argc_argv entry,
# pthreads, wasm features). Step 2 below lowers it with clang-23.
...
# Odin wasm: emit freestanding_wasm32 IR -> emcc lowers -> emcc link + joltc + Jolt.
...
# $(call make_app_bundle,<exe-path>,<app-path>). The quarantine strip matters:
# browser-downloaded asset BMPs carry com.apple.quarantine, which makes Launch
# Services refuse the signed bundle (AppKit aborts in _RegisterApplication).
...
# Bundles the clang-23 build so all ports share one codegen family. Apple-clang
# stays available: `make cpp-apple`, or `make app CPP_APP_BIN=$(TARGET)`.
...
# --- Zig native build -------------------------------------------------------
# joltc archive, shared by the Zig and Odin native links.
...
# Zig native: emit IR (-lc -lc++ for the libc entry + Jolt's C++ runtime) ->
# clang-23 lowers -> link Jolt + joltc + Cocoa.
...
# --- Odin native via LLVM 23 (split IR interchange): Odin -o:speed IR, clang-23
# does the final NEON codegen. ~1% over Odin's own LLVM 22 backend. (llvm-ir
# writes <basename>.ll to the CWD -> mv.)
...
# Bundles the clang-23 split build. Stock backend: `make odin-bin`, or
# `make odin ODIN_APP_BIN=$(ODIN_BIN)`.
...
# All four bundles — run before comparative benchmarks to avoid stale bundles.
...
# NSHighResolutionCapable=false: gives a logical-res surface the window server
# upscales (no per-frame CPU upscale), matching the IOSurface backends. Editing
# the plist breaks the signature, so re-sign after.
